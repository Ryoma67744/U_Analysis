...
SKIPS_ARE_IMMATERIAL = {
    ...
    ("peak_annotation.py", "parse_scils_name"):
        ...
        "情報は落ちない",
    ("deg_utils.py", "read_annotation_overlay"):
        "ver52.5 ②: 保存済み注釈の重ね合わせ元を読む。握りつぶしは 2 種類あり、"
        "性質が違う:\n"
        "  (1) 行ごとの `continue` — 文字列でない値・数値だけ・gene と同一の注釈を"
        "      **意図的に除外**している（`is_meaningful_annotation` の既存判定）。"
        "      これは失敗ではなく仕様なので報告しない\n"
        "  (2) ファイル単位の `except` — 候補パスが 2 つ（`method_dir` と"
        "      `method_dir.lower()`）あり、1 つ目が読めなければ 2 つ目を試す。"
        "      これは正当な継続\n"
        "★ 残る穴: 両方読めなかったとき、利用者から見た症状は「注釈を保存していない」"
        "  と同じになる（ログには warning が出る）。`load_deg_results` は"
        "  レコードの list を返すだけで**警告を渡す口が無い**ため、ここで報告できない。"
        "  本表そのものは必ず返るので誤った値にはならないが、"
        "  通知の口を作るときはここも繋ぐこと（同じ理由で下の `load_deg_results` も"
        "  この登録簿に入っている）",
