...
2. **移動先**: DESI生データ / TIMS生データ / 解析出力 / アプリ内部データ から選ぶ。必要ならサブフォルダ名も指定
3. **「📦 移動する」** → 確認モーダルでファイル数・容量を確認して実行

移動先は上表の 4 か所に限定されます。上書きは行わず、同名フォルダがある場合はエラーになります。
解析の実行中は移動できません。

移動後、結果フォルダ内の `_project_meta.json` をもとに `projects.json` の参照先パスを自動更新します
（登録済みならパス更新、未登録なら復元）。移動直後の初回オープンは Seurat 抽出キャッシュが効かず
再抽出が入りますが、2 回目以降は通常速度に戻ります。
