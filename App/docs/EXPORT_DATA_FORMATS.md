# エクスポートデータ 形式説明書（インタラクティブ画面 / MetaboAnalyst / imzML）

本書は、MSI 解析アプリが出力する各データファイルの**形式・列/行の配置・値の意味**を、DESI / TIMS の両方について詳述する。

ver69.0 で追加した imzML 出力は、解析設定サイドバーの「🧬 imzML 入出力」から行う独立した操作です。
既存のインタラクティブ画面「データ出力」の形式選択には追加されていません。

| 操作 | 出力 | 強度の由来 |
|---|---|---|
| インタラクティブ画面「データ出力」（TIMS） | Parquet / CSV / xlsx | 入力テーブルの保存値（選択時はグループ平均） |
| インタラクティブ画面「データ出力」（DESI） | xlsx | 元 `.txt` の測定値 |
| 解剖×クラスタ「④ 解析用データ出力 (ZIP)」 | MetaboAnalyst 用 CSV 等の ZIP | RDS の測定アッセイから ROI×クラスタ等の平均 |
| サイドバー「🧬 imzML 入出力」 | imzML / ibd / manifest の ZIP | imzML から登録した Parquet の保存値（画素単位） |

> **使い方（生成AIに渡す場合）**
> 下の各「■ コピー用ブロック」を**丸ごとコピー**し、出力データファイルと一緒に生成AIへ添付すると、
> AI が各列・各行・各値の意味を正確に解釈できる。ブロックは**それぞれ自己完結**しているので、
> 扱うファイルに応じて必要なブロック（TIMS / DESI / MetaboAnalyst / imzML）だけをコピーすればよい。
...
- **annotation** = 変換済みデータ由来のスポット注釈。切片/サンプルの識別に使う。imzML 登録では全画素 `Unannotated`。
- **領域名 / ROI** = 「解剖×クラスタ」で MSI または H&E にユーザーが描いた領域ラベル（例 `Brain`）。annotation とは別物。
...
新形式のクラスタ・群の付与には元ファイルID＋元画素IDを使います。旧結果とMSI/H&E領域名には従来のサンプル＋座標照合を使用します。`Sample` や元の `annotation` の表示名は変更しません。
...
■ 列の並び（左から。この順で格納される）
  ...
  [2]   x           : 空間X座標（数値）
  [3]   y           : 空間Y座標（数値）
  [4..N]<m/z 列群>   : 各 m/z の強度（数値）。入力の精度・データ規模に依存。
                       列名は「化合物名付き」(例: Glutathione_611.1439 | [M-H]-)
                       または m/z 数値文字列(例: 611.143900)。値=そのスポットでのその m/z の強度。
  [N+1] annotation   : スポットの注釈ラベル（文字列）。imzML登録では Unannotated。
  ...
  [最終列] 領域名     : 解剖×クラスタで割り当てた MSI/H&E の ROI 領域名（例 Brain）。未割当は空欄。

■ 値・突合の注意
  ・m/z強度 = 入力テーブルの保存値。入力がRMS等で正規化済みなら、その値を保持する。
  ・新形式のクラスタ・群は元ファイルID＋元画素IDで照合。旧結果とMSI/H&E領域名は従来のサンプル＋座標照合。
  ...
  ・「annotation」(入力由来スポット注釈) と「領域名」(MSI/H&E ROI) は別物。
  ・領域名は 解剖×クラスタで ROI を spot に割り当てて初めて値が入る（未設定なら列はあるが全空欄）。
```
...
■ (b) グループ平均のときのファイル名と列
  ...
  [1..k]  グループキー列 : 選んだキー。以下から 1 つ以上を組み合わせる。
            ...
            → 「クラスタのみ」「切片×クラスタ」「切片×ROI」
              「切片×ROI×クラスタ」が作れる。
            ※ 領域名をキーに含める場合は ROI の割当が必要。MSI に直接描く場合は H&E 画像は不要。
...
■ 値の意味
  ・強度は**入力の保存値**（変換済み parquet の値）。Seurat の正規化は通らない。
...
■ DESI では出力形式・出力内容を選べない（ver62.2 で明示）
  DESI の出力は元 .txt のレイアウトをそのまま保つ形式なので、
  「出力形式」(Parquet/CSV/xlsx) と「⚙ 出力内容の設定」(列の選択・グループ平均・
  m/z 一覧) は **DESI には適用されない**。
  列を選びたい・平均を取りたい場合は TIMS 形式(parquet)のデータを使う。

  ver62.3: 画面でこの 2 つが隠れるのは、装置を**断定できるとき**だけ
  （データフォルダの中身が DESI 固有／プロジェクト設定で明示的に DESI を選択）。
  パスに `DESI` という階層があるだけ、といった推測では隠さない。
  推測が外れると利用者が形式も列も選べなくなり、損害が大きすぎるため。
  隠さずに DESI 出力へ入った場合は、結果のメッセージに
  「適用していません」と出る。
...
■ 各データ行の列の並び（左から）
  ...
  [最終列] 領域名     : 解剖×クラスタの MSI/H&E ROI 領域名。未割当スポットは空欄。
...
■ 値・突合の注意
  ...
  ・元.txt の「ROIラベル列」(DESI固有) と アプリ追加の「領域名」(MSI/H&E ROI) は別由来。
  ・ver62.2: 生データの拡張子は大文字小文字を区別しない（`SAMPLE.TXT` も対象）。
```

> **どちらの形式で出るかは「データフォルダの中身」で決まる（ver62.2）**
> parquet が入っていれば TIMS、`.xlsx/.xls` が入っていれば DESI。
> `.txt` / `.csv` はどちらでも使われるため、プロジェクト設定の装置 →
> フォルダのパス (`Data/DESI/Data` ・ `Data/TIMS/Data`) の順で決める。
> ver62.1 以前は**パスだけ**で決めていたため、parquet を `Data/DESI/…` に置くと
> DESI 経路に入り「DESI .txt ファイルが見つかりません」で出力できなかった。
> 入力が見つからないときのエラーには、探した場所・そこにあった拡張子・
> 装置の判断根拠が出る。
...
ver68.0 以降、MSI（TIC）に直接描いた領域も集計できます。MSI 領域は H&E 画像・位置合わせ不要です。
H&E に描いた領域を混在させ、位置合わせが未完了の場合は、その H&E 領域を除外したことが画面と ZIP に示されます。
...
## ■ コピー用ブロック 4 : サイドバー「🧬 imzML 入出力」（ver69.0）

操作手順は [IMZML_IO.md](IMZML_IO.md) を参照。

```text
【MSI解析アプリ 出力ファイル形式：imzML 入出力の ZIP】

操作場所: 解析設定サイドバー「🧬 imzML 入出力」
操作: 登録済み Parquet → imzML + ibd ZIP
入力: このツールで imzML から登録した sample.parquet と、同じ場所の sample.imzml.json。
      CSV由来のParquetやインタラクティブ画面の結果表は、必要な登録情報がないため対象外。
出力: ユーザーが指定したサーバー上の .zip ファイル（SFTP等で取得）。

■ ZIP 内のファイル
  sample.imzML        : m/z軸、画素座標、外部バイナリ参照など。
  sample.ibd          : m/z・強度のバイナリデータ。imzMLと同じ場所で扱う。
  export_manifest.json: 元Parquet、画素数、m/z数、指定ID（全画素なら "all"）、正規化状態。

■ 画素選択
  空欄 = 全画素。指定時は登録後Parquetのidをカンマ区切りで入力。
  id は (y, x) 順に並べた後、1から付け直した連番。
  元imzMLのスペクトル順、SpotIndex、UMAP CellIDとは同一とは限らない。
  「1,2,3,4」はID 1～4を意味し、2×2 ROIを自動で指定する値ではない。
  ROI/クラスタ選択画面からの自動連携はない。元Parquetのid/x/yとの対応確認が必要。

■ 値と対応範囲
  ・強度は登録Parquetの保存値。RDSの正規化値やグループ平均には置き換えない。
  ・保存値の正規化状態は unknown。SCiLSでRMS等を適用済みかは元の出力条件を確認する。
  ・元のm/z軸、座標、数値精度に基づいて生成する。全元メタデータの複製ではない。
  ・全画素共通m/z軸、単一z面、float32/float64強度が対象。
  ・ROI名、クラスタ、UMAP座標を付加した表ではない。
  ・既存「データ出力」の列選択・集計・形式指定やGPT APIからの形式指定は適用されない。
  ・初期実装。SCiLS実データでの往復検証は未完了。
```

---

## 付録: SCiLS CSV から変換した MSI parquet（TIMS 出力の元データ）

この付録の row group・注釈・メタデータの説明は **SCiLS CSV 変換器**の仕様です。
ver69.0 の imzML 登録は別の変換経路で、Parquet に加えて `.imzml.json` を生成し、
既定で最大128画素ごとの row group に書き込みます。両者の所要時間やファイルサイズは同一とは限りません。
