...
### MetaboAnalyst 出力: 強度選択の意味を UI に明記（挙動不変）

TIMS は SCiLS 書き出し時点で RMS 補正済みのため、パイプラインは `data = log1p(counts)` となり
**「線形化」= expm1(data) = counts = 「生 counts」（同値）**。UI の「線形化(推奨)」表記が
「再正規化を勧める」ように誤解されるのを避けるため、文言を明確化した（数値・挙動は不変）。

- `layouts/hne_overlay_tab.py`: 強度セレクタのラベルを「線形化 (非log・推奨)」→「線形化 (非log)」に変更。
  説明文に「非log で渡すのが無難（『現状(log)』は二重logで非推奨）」「**SCiLS RMS 済みの TIMS では
  線形化＝生counts（同値）**」「DESI では別物」を明記。
- 既定値（線形化）・出力ロジックは不変。実際の前処理は出力後ステータスの `preprocessing:` にも表示される。
- version 38.0→38.1。
