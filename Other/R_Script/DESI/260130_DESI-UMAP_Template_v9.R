  ...
  for (cl in sort(unique(deg_markers$cluster))) {
    tryCatch({
      ...
      p <- ggplot(df_sub, aes(x = avg_log2FC, y = log_p, col = color_group)) +
        ...
        ggrepel::geom_text_repel(aes(label = label_text), size = 3.0, max.overlaps = Inf) +
      ...
      p_mrm <- ggplot(df_sub, aes(x = avg_log2FC, y = log_p, col = color_group)) +
        ...
        geom_vline(xintercept = c(-DEG_LOGFC_TH_VAL, DEG_LOGFC_TH_VAL), linetype = "dashed") +
        geom_hline(yintercept = -log10(DEG_P_THRESH_VAL), linetype = "dashed") +
        ggrepel::geom_text_repel(aes(label = label_mrm), size = 2.5, max.overlaps = Inf, box.padding = 0.5, force = 2) +
...
read_desi_data <- function(file_path, sample_prefix = NULL) {
  ...
  if (length(lines) < 5) stop("データファイルの行数が不足しています: ", file_path)
  lines <- lines[nzchar(lines)]
  metabolite_numbers <- trimws(strsplit(lines[2], "\t")[[1]])
  pre_masses <- trimws(strsplit(lines[3], "\t")[[1]])
  post_masses <- trimws(strsplit(lines[4], "\t")[[1]])
  ...
  if (length(data_lines) == 0) stop("データ行が存在しません: ", file_path)
  
  data_list <- vector("list", length(data_lines))
  for(i in seq_along(data_lines)) {
    split_line <- trimws(strsplit(data_lines[i], "\t")[[1]])
    split_line <- split_line[nzchar(split_line)]
    data_list[[i]] <- split_line
  }
  
  max_cols <- max(sapply(data_list, length))
  data_matrix <- matrix(NA, nrow = length(data_lines), ncol = max_cols)
  for(i in seq_along(data_list)) {
    data_matrix[i, 1:length(data_list[[i]])] <- data_list[[i]]
  }
...
if (length(seu_list) == 1) {
  ...
  if (RESUME_FROM_RDS && file.exists(rds_path_single_in)) {
    ...
    seu_single <- ScaleData(seu_single)
    ...
    seu_single <- FindClusters(seu_single, resolution = 0.5)
...
dims_use_rpca <- get_safe_dims_for_rpca(seu_list_pca, max_dims = 30, reduction = "pca")
    ...
    seu_rpca <- FindClusters(seu_rpca)
