...
V13_SCRIPT_PATH <- "C:\\Users\\Cciia\\Biochem Dropbox\\木津亮馬\\UMAP\\Other\\R_Script\\TIMS\\260125_DBSCAN_With_cluster_ver13.R"
...
if (isTRUE(ENABLE_REUMAP_REPLACE)) {
  ...
  if (!nzchar(rerun_rds)) {
    ...
    project_label <- paste0(V13_PROJECT_LABEL_PREFIX, suffix_run, "_")
    # ver13 は OUTPUT_DIR 内にサブディレクトリ（PROJECT_LABEL_YYYYMMDD）を作成する
    # project_label は L1102 で定義済み、today は L1100 で定義済み
  ...
  # UMAP exists?
  .stopif("umap" %in% names(seu@reductions), "元データ側に 'umap' reduction がありません（RDSにUMAPが入っているStep2/Step3を指定してください）。")
...
if (.should_merge && nzchar(MERGE_SCRIPT_PATH) && file.exists(MERGE_SCRIPT_PATH)) {
  ...
  if (!nzchar(merge_rerun_rds)) {
    if (isTRUE(RUN_V13_AFTER_EXPORT)) {
      today_m <- format(Sys.Date(), "%Y%m%d")
      suffix_m <- paste0("keep_", paste(TARGET_CLUSTERS, collapse = "-"))
      plabel_m <- paste0(V13_PROJECT_LABEL_PREFIX, suffix_m, "_")
      od_run <- file.path(V13_OUTPUT_DIR, paste0(plabel_m, "_", today_m))
  ...
  if (nzchar(merge_rerun_rds) && file.exists(merge_rerun_rds)) {
    ...
    SAMPLE_NAME_MAP    <- SAMPLE_NAME_MAP  # ver13 で定義済みの場合はそれを使用
